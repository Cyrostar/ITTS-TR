...
# --- ITTS-TR INTERNAL IMPORT FIX ---
# Appends the rvc/train folder to sys.path so it can find its sibling scripts (losses.py, utils.py)
...
def run(
    ...
    # --- ITTS-TR GLOO HIJACK FIX ---
    # Gloo network binding is notoriously broken on Windows. 
    # Since we are using a single GPU, we completely bypass the distributed network initialization!
    if n_gpus > 1:
        os.environ["MASTER_ADDR"] = "127.0.0.1"
        os.environ["MASTER_PORT"] = os.environ.get("MASTER_PORT", "23333")
...
def train_and_evaluate(
    ...
    Args:
        ...
        config (Namespace): Hyperparameters (changed from hps to config to match scope).
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        for batch_idx, info in data_iterator:
            ...
            with torch.amp.autocast(
                ...
                # --- IMMUNITY FIX 2.0: PRE-FORWARD VALIDATION SHIELD ---
                # 1. Clamp embeddings to prevent NaN/-2147483648 crashes from corrupted pitch extractors
                pitch = torch.clamp(pitch, min=0, max=255)
                sid = torch.clamp(sid, min=0, max=config.model.spk_embed_dim - 1)

                # 2. Guarantee Minimum Tensor Lengths to prevent internal Generator Gather crashes
                min_frames = config.train.segment_size // config.data.hop_length
                if spec.size(2) < min_frames:
                    pad_frames = min_frames - spec.size(2)
                    spec = torch.nn.functional.pad(spec, (0, pad_frames))
                    phone = torch.nn.functional.pad(phone, (0, pad_frames))
                    pitch = torch.nn.functional.pad(pitch, (0, pad_frames))
                    pitchf = torch.nn.functional.pad(pitchf, (0, pad_frames))
                    
                if wave.size(2) < config.train.segment_size:
                    pad_wave = config.train.segment_size - wave.size(2)
                    wave = torch.nn.functional.pad(wave, (0, pad_wave))
                # -------------------------------------------------------
                # Forward pass
                model_output = net_g(
                ...
                # --- IMMUNITY FIX: DYNAMIC WAVE TENSOR PADDING ---
                # Prevents CUDA out-of-bounds crashes when hop_length frames misalign
                max_idx = int(ids_slice.max().item())
                max_required_len = (max_idx * config.data.hop_length) + config.train.segment_size
    ...
    if rank == 0:
        ...
        if randomized:
            ...
            # --- IMMUNITY FIX: DYNAMIC MEL TENSOR PADDING ---
            max_mel_idx = int(ids_slice.max().item())
            max_mel_required = max_mel_idx + (config.train.segment_size // config.data.hop_length)
            if mel.size(-1) < max_mel_required:
                pad_size_mel = max_mel_required - mel.size(-1)
                mel = torch.nn.functional.pad(mel, (0, pad_size_mel))
            # ------------------------------------------------
