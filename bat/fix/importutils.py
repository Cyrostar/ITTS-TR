...
class LazyModule(ModuleType):
    ...
    def ensure_module(self, stacklevel: int) -> ModuleType:
        ...
        # --- RECURSION SAFE FIX START ---
        # We check the frame manually to avoid calling inspect.getframeinfo,
        # which causes an infinite loop during UI/debugger inspection on Windows.
        try:
            _frame = sys._getframe(stacklevel + 1)
            _filename = _frame.f_code.co_filename
            _name = _frame.f_globals.get("__name__", "")

            if _filename.endswith("inspect.py") or _name == "inspect":
                raise AttributeError("SpeechBrain: Aborting lazy-load during inspection.")
        except (AttributeError, ValueError):
            pass
        # --- RECURSION SAFE FIX END ---
...
class DeprecatedModuleRedirect(LazyModule):
    ...
    def ensure_module(self, stacklevel: int) -> ModuleType:
        ...
        # can fail with exception if the module shouldn't be imported
...
def lazy_export(name: str, package: str):
    """Makes `name` lazily available under the module list for the specified package."""
    if hasattr(sys.modules[package], name):
        return
    setattr(sys.modules[package], name, LazyModule(name, name, package))


def lazy_export_all(init_file_path: str, package: str, export_subpackages: bool = False):
    """Makes all modules under a module lazily importable."""
...
def deprecated_redirect(
    ...
    """Patches the module list to add a lazy redirection from `old_import` to `new_import`."""
