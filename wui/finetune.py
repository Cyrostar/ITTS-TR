...
def resume_finetune_ui(proj, config, tokenizer, train, val, vec, name, _, epochs, bs, lr, accum, workers, dur, drop, save, use_compile, case_format, tok_type, vocab_type, resize_vocab, expand_text, expand_audio, lang, wordify, abbrev, extract):
    if not name.strip():
    ...
    if os.path.exists(interrupted_path):
        ...
        yield from finetune_official_ui(proj, config, tokenizer, train, val, vec, name, resume_path, epochs, bs, lr, accum, workers, dur, drop, save, use_compile, case_format, tok_type, vocab_type, resize_vocab, expand_text, expand_audio, lang, wordify, abbrev, extract)
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    ...
                    vocab_type_dd = gr.Dropdown(
                        label=_("FINETUNE_LABEL_VOCAB_TYPE"), 
